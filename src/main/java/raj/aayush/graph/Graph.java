...
import raj.aayush.functional.ThreeArgumentStatement;
import raj.aayush.functional.TwoArgumentStatement;
import raj.aayush.heap.LinkedHeap;
import raj.aayush.linked.list.LinkedList;
import raj.aayush.linked.list.functional.OneArgumentExpression;
...
public interface Graph<V, E> {
	...
	default void visitAllConnectedEdges(int startingNode, ThreeArgumentStatement<Integer, Integer, E> visitor,
		...
		switch (type) {
		case BFT:
			toBeProcessed = new QueueImplLinkedList<Edge>();
			break;
		case DFT:
			toBeProcessed = new StackImplLinkedList<Edge>();
			break;
		}
	...
	default boolean detectCycle() {
		...
		try {
			if (isUndirected()) {
				...
				while (allVertices.getRoot() != null) {
					checkDirectedCycleFromVertex(LinkedList.<Integer>emptyList()
							.add(allVertices.getRoot().getValue()), allVertices);
				}
			}
	...
	default void checkDirectedCycleFromVertex(LinkedList<Integer> path,
	...
	default LinkedList<Edge> minimumSpanningTree(OneArgumentExpression<E, Integer> costFinder) {
		...
		this.getAllVertices().traverseDepthFirstNonRecursive((x) -> unionFind.add(x),
