...
public class RedBlackTree<E extends Comparable<E>> extends BinarySearchTree<E> {
	...
	protected BinaryTree.Node<E> newNode(BinaryTree.Node<E> parent, BinaryTree<E> containerTree, E value) {
		return new Node<E>(parent, containerTree, value);
	}
	...
	protected void rebalanceForInsert(Node<E> node) {
		if (node.getParent() == null) {
			...
			if (parent.black) {
				...
				Node<E> uncle = nodeLeftGrandChild ? (Node<E>) grandParent.getRight() : (Node<E>) grandParent.getLeft();
				if (!nullSafeBlack(uncle)) {
					...
					boolean middleChild = nodeLeftGrandChild ? parent.getRight() == node : parent.getLeft() == node;
					if (middleChild) {
			...
			if (sibling.black) {
				if (nullSafeBlack(nearNephew) && nullSafeBlack(awayNephew)) {
					...
					if (parent.getParent() != null) {
						rebalanceForDelete((Node<E>) parent.getParent(), parent.getParent().getLeft() == parent);
					}
	...
	protected void checkSanity(Node<E> node) {
		...
		int rightHeight = nullSafeBlackHeight((Node<E>) node.getRight());
		if (leftHeight != rightHeight) {
			throw new IllegalStateException("Black height does not match");
		} else if (!node.black) {
			if (!nullSafeBlack((Node<E>) node.getLeft()) || !nullSafeBlack((Node<E>) node.getLeft())) {
				displayText(node, 0);
